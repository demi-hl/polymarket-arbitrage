...
const MAX_CONCENTRATION_PCT = 0.20;
const VAR_LIMIT_PCT = 0.05;
...
class RiskManager {
  ...
  check(opportunity, requestedSize) {
    ...
    if (positions.length >= MAX_OPEN_POSITIONS) {
      return { allowed: false, reason: `Max open positions (${MAX_OPEN_POSITIONS})`, suggestedSize: 0 };
    }
    ...
    // Market-level concentration check
    const marketId = opportunity.marketId;
    if (marketId) {
      const marketExposure = positions
        .filter(p => p.marketId === marketId)
        .reduce((s, p) => s + (p.entryCost || 0), 0);
      if (equity > 0 && (marketExposure + size) / equity > MAX_CONCENTRATION_PCT) {
        return { allowed: false, reason: `Market concentration limit (${(MAX_CONCENTRATION_PCT * 100)}%)`, suggestedSize: 0 };
      }
    }
    ...
    // VaR check: 95% VaR must not exceed limit
    const volatility = opportunity.volatility || DEFAULT_VOLATILITY;
    const var95 = size * volatility * 1.645;
    const varLimit = equity * VAR_LIMIT_PCT;
    if (var95 > varLimit) {
      size = Math.min(size, varLimit / (volatility * 1.645));
    }
