 ...
 *
 * Layered risk controls that go beyond simple take-profit/stop-loss:
 *
 *   1. Kelly Criterion Sizing — optimal bet size based on estimated edge
 *      and win probability, capped at fractional Kelly for safety
 *   2. Correlation Limits — prevent overexposure to similar markets
 *   3. Max Drawdown Circuit Breaker — pause trading if equity drops
 *      below a threshold from its peak
...
const MAX_PORTFOLIO_RISK_PCT = 0.80;
const MAX_SINGLE_POSITION_PCT = 0.05;
const MAX_SECTOR_PCT = 0.30;
const CIRCUIT_BREAKER_DRAWDOWN = 0.15;
const DAILY_LOSS_LIMIT_PCT = 0.10;
const MAX_CORRELATED_POSITIONS = 3;
...
class RiskManager {
   ...
   * We use fractional Kelly (MAX_KELLY_FRACTION) for safety.
   */
  kellySize(edge, winProb, bankroll) {
    if (edge <= 0 || winProb <= 0 || winProb >= 1) return 0;
    const b = (1 + edge) / 1 - 1 || edge;
    const q = 1 - winProb;
    const kelly = (b * winProb - q) / (b || 1);
    if (kelly <= 0) return 0;
    const fractional = kelly * MAX_KELLY_FRACTION;
    return Math.max(0, Math.min(fractional * bankroll, bankroll * MAX_SINGLE_POSITION_PCT));
  }

  /**
   * Check if a new opportunity passes all risk gates.
   * Returns { allowed, reason, suggestedSize }.
   */
  check(opportunity, requestedSize) {
    ...
    const similarPositions = positions.filter(p => {
      ...
      if (p.strategy === opportunity.strategy) return true;
      const pCat = (p.question || '').toLowerCase();
      const oCat = (opportunity.question || '').toLowerCase();
      const words = oCat.split(/\s+/).filter(w => w.length > 4);
      return words.some(w => pCat.includes(w));
    });
    ...
    const winProb = opportunity.confidence || 0.6;
